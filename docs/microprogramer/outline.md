...
* 我是一名前端工程師，主要使用 Vue 3、Nuxt 3 與 TypeScript。
* 近四年來，我參與過企業內部系統、品牌官網與 SaaS 產品開發。
* 在成為前端工程師之前，我曾擔任 PM。那段經驗讓我習慣先理解需求、流程與協作成本，再開始思考技術實作。
* 我關心的不只是畫面完成，也包含資料狀態、元件責任、使用體驗與後續維護。
* 近期我也將 Cursor 等 AI 工具導入日常開發，協助理解程式、拆解功能、Debug 與整理文件。
* AI 可以加速，但最後的架構、型別、邏輯與邊界情境，仍然需要工程師自己判斷。
...
這頁語氣可以比較像 marimba.design 的 About：先讓面試官感覺到「我是什麼樣的工程師」，再慢慢進入技術細節。重點是自然、自信，不要像背履歷。
...
A path shaped by products, teams, and code.

### 呈現方式

使用 Timeline，讓每一段工作經歷都對應一個能力累積。版面可以保留年份、公司、角色，再用一句短文說明那段經驗帶給我的能力。

### 內容方向

1. **我瘋程式工作室｜PM**
   * 從需求訪談、時程規劃到設計／開發協調，第一次近距離看見一個網站專案如何被交付。
   * 也因為這段經驗，我開始意識到技術能力的重要性，後來主動自學並轉職前端。
2. **奇點無限｜前端工程師**
   * 在技術長帶領下參與客戶系統重構，累積 codebase 維護、模組化改寫與 Code Review 經驗。
   * 也曾承接離職工程師留下的既有專案，先理解功能與 API，再繼續推進下一期交付。
   * 與前端同事協作建置共用元件庫，統一設計規範與元件 API。
3. **勝崨有限公司｜前端工程師**
   * 參與企業 B 端系統、Nuxt 企業官網與 SaaS 產品功能迭代。
   * 與 SA、PM、Backend、UI/UX 協作，從需求對齊、功能落地到 UAT 與上線都有參與。
4. **JOSUI｜Side Project**
   * 個人發起的 shadcn/ui 色彩主題預覽工具。
   * 用 AI 協作完成元件設計、開發與部署，也練習如何審查 AI 產出，而不是直接照單全收。

### 口頭重點

這頁可以用比較故事性的方式講：「我的前端能力不是只從技術清單長出來的，而是從專案交付、既有系統維護、跨角色合作和 side project 中慢慢成形。」
...
The next place I want to grow.

### 內容方向

* 我希望下一個階段能更深入參與產品與系統開發，而不只是完成單一畫面。
...
我期待下一個階段，可以更深入參與產品與系統開發。

過去的工作讓我累積了 Vue、Nuxt、TypeScript、企業系統、Nuxt 官網、SaaS 產品、共用元件與跨角色協作經驗。JOSUI 也讓我實際練習如何用 AI 協助開發，同時保留工程師對架構與品質的判斷。

我看到微程式很重視產品開發流程、AI 工具應用、Code Review 與高品質交付。這些方向和我目前想深化的前端工程能力很接近，也會是我希望繼續投入的地方。

### 注意事項

語氣要真誠，不要過度稱讚公司。重點放在「我過去做過什麼」和「這些經驗如何對應微程式正在找的人」。
...
```text
I build frontend experiences with product context, maintainable structure, and thoughtful collaboration.
```

或使用中文版本：

```text
我希望把需求、技術與使用體驗，落成穩定且好維護的前端產品。
```
